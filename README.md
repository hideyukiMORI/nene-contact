...
- 🚧 **AI / MCP (M6)** — agent read surface `/api/*` (machine-key auth, redacted by default, audited `include_pii`), a local **MCP stdio server** (`tools/local-mcp-server.php`, [docs](./docs/integrations/mcp.md)), **Concierge ingest** (`POST /api/submissions`, `source=concierge` → one inbox), and **MCP write tools** behind a two-step confirmation token; Invoice/Records siblings next.
