...
- 🚧 **AI / MCP (M6)** — agent read surface `/api/*` (machine-key auth, redacted by default, audited `include_pii`), a local **MCP stdio server** (`tools/local-mcp-server.php`, [docs](./docs/integrations/mcp.md)), and **Concierge ingest** (`POST /api/submissions`, `source=concierge` → one inbox); MCP write tools + Invoice/Records next.
