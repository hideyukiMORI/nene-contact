...
| `admin_notification_subject` / `admin_notification_body` | `contact_form` | per-form admin-notification email template; null = Japanese default. Variables (admin only): reserved `{form_name}` `{submitted_at}` `{message}` + each non-honeypot field by its `name` (e.g. `{company}`); reserved names win; unknown `{token}` left literal (email-wording wave c, #446; dynamic #450) |
