...
only, never a shared database (ADR 0002). ✅ **Done** — Deal opportunity handoff (#112) +
Vault attachment archive (#116).
...
- [x] **Contact → Vault**: upload a submission attachment as a received document (DO D12,
      Phase 3+). (#116 — `VaultClientInterface`, multipart upload, per-attachment link)
...
- [x] Environment variables per sibling-products (`NENE_DEAL_*`, `NENE_VAULT_*`); service
      tokens sent as bearer credentials, never logged. (#112 Deal + #116 Vault)
