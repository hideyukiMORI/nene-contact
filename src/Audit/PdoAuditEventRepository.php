...
final readonly class PdoAuditEventRepository implements AuditEventRepositoryInterface
{
    ...
    {
        $this->query->execute(
            'INSERT INTO audit_events (actor_user_id, organization_id, action, entity_type, entity_id, before_json, after_json, created_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
            [
                $event->actorUserId,
                ...
                $event->entityType,
                $event->entityId,
                self::encode($event->before),
                self::encode($event->after),
                date('Y-m-d H:i:s'),
            ],
        );
