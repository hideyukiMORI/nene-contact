...
use NeneContact\Handoff\HandoffToDealUseCase;
...
final class HandoffToDealUseCaseTest extends TestCase
{
    ...
    public function test_success_stores_opportunity_id_and_audits_created(): void
    {
        $submission = new Submission(organizationId: 7, contactFormId: 2, fieldValues: ['email' => 'visitor@example.com'], status: 'open', id: 9);
        $links = new InMemorySubmissionLinkRepository();
        $deal = new class () implements DealClientInterface {
        ...
        $useCase = new HandoffToDealUseCase($this->submissions($submission), $this->forms(), $links, $deal, $this->audit($capture));
        $link = $useCase->execute(5, 9);

        self::assertSame(SubmissionLink::STATUS_SUCCEEDED, $link->handoffStatus);
        self::assertSame('opp_123', $link->dealOpportunityId);
        self::assertNull($link->lastError);
        self::assertSame('9', $deal->externalReference); // idempotency key = submission id
        self::assertSame('visitor@example.com', $deal->received['fields']['email'] ?? null);
        self::assertCount(1, $links->links);

        self::assertCount(1, $capture['events']);
        self::assertSame('handoff.created', $capture['events'][0]->action);
        // The audit trail carries ids, never raw visitor PII.
    ...
    public function test_upstream_failure_is_non_destructive_and_records_failed(): void
    {
        ...
        $deal = new class () implements DealClientInterface {
            public function createOpportunity(string $externalReference, array $payload): string
            {
                throw new UpstreamRequestException('Deal handoff failed with status 503.');
            }
        };
        $capture = ['events' => []];
        ...
        self::assertNull($link->dealOpportunityId);
        self::assertSame('Deal handoff failed with status 503.', $link->lastError);
        self::assertCount(1, $capture['events']);
        self::assertSame('handoff.created', $capture['events'][0]->action);
    }
    ...
    public function test_retry_after_failure_succeeds_and_audits_retried(): void
    {
        $submission = new Submission(organizationId: 7, contactFormId: 2, fieldValues: ['email' => 'visitor@example.com'], status: 'open', id: 9);
        $links = new InMemorySubmissionLinkRepository();
        $links->save(new SubmissionLink(organizationId: 7, submissionId: 9, target: SubmissionLink::TARGET_DEAL, handoffStatus: SubmissionLink::STATUS_FAILED, lastError: 'previous failure'));
        ...
        self::assertSame(SubmissionLink::STATUS_SUCCEEDED, $link->handoffStatus);
        self::assertSame('opp_777', $link->dealOpportunityId);
        self::assertNull($link->lastError);
    ...
    public function test_rejects_unknown_submission(): void
    {
        ...
        $useCase = new HandoffToDealUseCase($this->submissions(null), $this->forms(), $links, $deal, $this->audit($capture));
