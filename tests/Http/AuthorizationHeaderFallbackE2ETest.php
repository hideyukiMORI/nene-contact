 ...
 *
 * The operator console mirrors every bearer token into `X-Authorization: Bearer <token>`
 ...
 * {@see \NeneContact\Auth\AdminApiAuthMiddleware} runs.
 *
 * `GET /admin/organizations` is bearer-protected but bypasses org resolution (see
 * {@see \NeneContact\Organization\Resolution\OrgResolverMiddleware}), so these
...
final class AuthorizationHeaderFallbackE2ETest extends TestCase
{
     ...
     * fallback receiver and accepted by {@see \NeneContact\Auth\AdminApiAuthMiddleware}
     ...
     * would be downstream of auth, which is out of scope for the transport-level
     * mirror proof).
     */
